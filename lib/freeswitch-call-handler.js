...
const convertPcapToMp3 = require('./wav-to-mp3-converter');
const parseRecordingId = require('./recording-data-parser');
...
function handleIncomingSiprecInvite(req, res, opts) {
  ...
  return payloadParser(opts)
    ...
    .then(setDialogHandlers.bind(this, opts, opts.logger));
}
...
function setDialogHandlers(opts, logger, {uas, uac}) {
  uas
    .on('destroy', () => {
      logger.info('call ended normally');
      uac.destroy();

      const { subdirectoryName, recordingId } = parseRecordingId(
        opts.recordingData.recording.extensiondata[0]['jb:recordingid'][0]
      );
